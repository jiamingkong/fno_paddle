...
class MultigridPatching2D(nn.Layer):
    def __init__(
        self, model, levels=0, padding_fraction=0, use_distributed=False, stitching=True
    ):
        ...
        if levels > 0:
            print(
                f"MGPatching({self.n_patches=}, {self.padding_fraction=}, {self.levels=}, {use_distributed=}, {stitching=})"
            )
    ...
    def _stitch(self, x):
        ...
        assert (
            x.ndim == 4
        ), f"Only 2D patche supported but got input with {x.ndim} dims."
