...
def load_darcy_numpy(
    ...
    p = Path(data_path).joinpath(f"darcy_train_{train_resolution}.npy").as_posix()
    # load with numpy
    data = np.load(p, allow_pickle=True).item()
    x_train = (
        paddle.to_tensor(data["x"][0:n_train, :, :])
        .unsqueeze(channel_dim)
        .astype("float32")
        .clone()
    )
    y_train = (
        paddle.to_tensor(data["y"][0:n_train, :, :]).unsqueeze(channel_dim).clone()
    )
    del data
    ...
    p = Path(data_path).joinpath(f"darcy_test_{train_resolution}.npy").as_posix()
    # load with numpy
    data = np.load(p, allow_pickle=True).item()
    x_test = (
        paddle.to_tensor(data["x"][:n_test, :, :])
        .unsqueeze(channel_dim)
        .astype("float32")
        .clone()
    )
    y_test = paddle.to_tensor(data["y"][:n_test, :, :]).unsqueeze(channel_dim).clone()
    del data
    ...
    test_loader = paddle.io.DataLoader(
        test_db, batch_size=test_batch_size, shuffle=False, num_workers=0
    )
    ...
    for (res, n_test, test_batch_size) in zip(
        ...
        p = Path(data_path).joinpath(f"darcy_test_{res}.npy").as_posix()
        data = np.load(p, allow_pickle=True).item()
        x_test = (
            paddle.to_tensor(data["x"][:n_test, :, :])
            .unsqueeze(channel_dim)
        ...
        y_test = (
            paddle.to_tensor(data["y"][:n_test, :, :]).unsqueeze(channel_dim).clone()
        )
        del data
