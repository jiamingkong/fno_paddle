    ...
    def __init__(
        ...
        **kwargs,
            ...
            self.mlp = nn.LayerList(
                [
                    MLP(
                        ...
                        hidden_channels=int(
                            round(self.hidden_channels * mlp["expansion"])
                        ),
                        dropout=mlp["dropout"],
                        n_dim=self.n_dim,
                    )
        ...
        if norm is None:
            ...
            self.norm = nn.LayerList([nn.LayerNorm() for _ in range(n_layers)])
        else:
            raise ValueError(
        ...
        self.lifting = Lifting(
            in_channels=in_channels, out_channels=self.hidden_channels, n_dim=self.n_dim
        )
